...
int main( int argc, char* argv[] )
{
    ...
    if(N%numtasks==0){
        ...
        Matrix matrice_inter(N, Ncol);
        for(int i=0; i<Ncol; ++i){
            for(int j=0; j<N; ++j){
                matrice_inter(j,i) = (rank*Ncol+i+j)%N;
            }
        }

        std::vector<double> u(Ncol);
        for (int i = 0; i < Ncol; ++i ) u[i] = rank*Ncol+i+1;
        std::vector<double> partial_result = matrice_inter*u;

        //On somme les vecteurs intermédiaires
        std::vector<double> result(N);
        MPI_Allreduce(partial_result.data(),result.data(), N, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        
        std::cout << "Processus " << rank << " : result = " << result << std::endl;
    }
    else{
        std::cout << "Utiliser un nombre de processus diviseur de N \n";
    }
